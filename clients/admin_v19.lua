...
local function renderSensor(e,s,index,total)
 prep(e);header(e,"SENSOR "..index.."/"..total);if not s then center(e,7,"NO SENSOR",C.dim);return end;local ds=upper(s._telemetryStatus or"LIVE");put(e,2,5,nice(s.reportedName or s.name or s.type or"SENSOR"),C.text);if s._source then put(e,2,6,"NODE "..tostring(s._source)..(s._connected==false and"  NOT REACHABLE"or""),s._connected==false and C.warn or C.dim)end;rule(e,8);put(e,2,10,nice(s.summary or"ONLINE").."  "..ds,ds=="LIVE"and C.good or C.warn);local rows=metricRows(s);local y=12;for _,r in ipairs(rows)do if y+1>e.h then break end;put(e,2,y,r[1],C.dim);put(e,2,y+1,r[2],C.text);y=y+3 end;if y<=e.h and #rows==0 then put(e,2,y,"METHODS "..tostring(s.methodCount or 0),C.dim)end
end
