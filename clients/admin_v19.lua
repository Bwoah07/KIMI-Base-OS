...
local function renderFleet(e,env)
 prep(e);identifyTargets[e.name]={};header(e,"FLEET / IDENTIFY");local f=state(env).fleet or{};local ids={};for id in pairs(f)do ids[#ids+1]=id end;table.sort(ids,function(a,b)local na,nb=tonumber(a),tonumber(b);if na and nb then return na<nb end;return tostring(a)<tostring(b)end);put(e,2,5,"TOUCH A ROW TO IDENTIFY",C.dim);if identifyStatus~=""then put(e,2,6,identifyStatus,C.warn)end;local y=8;local now=os.epoch("utc");for _,id in ipairs(ids)do if y+1>e.h then break end;local m=f[id];local age=tonumber(m.ageMs)or(now-(tonumber(m.lastSeen)or0));local st=age<=10000 and"ONLINE"or(age<=45000 and"STALE"or"OFFLINE");local fg=st=="ONLINE"and C.good or(st=="STALE"and C.warn or C.bad);local label="ID "..tostring(id).." "..upper(m.name or m.role or"KIMI");put(e,2,y,label,C.text);put(e,math.max(2,e.w-#st-1),y,st,fg);put(e,2,y+1,upper(m.role or"?").."  "..tostring(m.version or"?"),C.dim);identifyTargets[e.name][#identifyTargets[e.name]+1]={y1=y,y2=y+1,id=tonumber(id)or id};y=y+3 end
end
