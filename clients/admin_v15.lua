...
local M={}
local cfg={}
local C={bg=colors.black,text=colors.white,dim=colors.lightGray,shell=colors.gray,good=colors.lime}
...
local function matrixPower(raw)
 raw=raw or{}
 local best,score=nil,-1
 for _,p in ipairs(raw.matrices or{})do
  if type(p)=="table"then local cap=tonumber(p.capacity)or 0;local stored=tonumber(p.stored)or 0;local s=cap*10+stored;if s>score then best,score=p,s end end
 end
...
local function fmtDuration(sec)
 sec=tonumber(sec)
 if not sec or sec<0 or sec~=sec then return"?"end
 sec=math.floor(sec+.5)
 ...
 if h<24 then return string.format("%dh %02dm",h,m)end
...
local function powerStatus(p)
 ...
 local activity=math.max(math.abs(input),math.abs(output),1)
 local threshold=math.max(1,activity*.002)
 ...
 if net>0 then
  if stored>=cap then return"FULL"end
  return"FULL IN "..fmtDuration((cap-stored)/(net*20))
 end
 if stored<=0 then return"EMPTY"end
 return"EMPTY IN "..fmtDuration(stored/((-net)*20))
end
...
local function redrawPower(e,env)
 ...
 local p=matrixPower(state(env).power or{});local pp=pct(p)
 center(e,5,pp and string.format("%.1f%%",pp)or"NO MATRIX",pp and C.good or C.dim,nil,2,e.w-1)
 -- Plain narrow Matrix tank: gray border, black empty area, lime fill bottom-up.
 local bodyW=math.min(9,math.max(7,e.w-16));local x1=math.floor((e.w-bodyW)/2)+1;local x2=x1+bodyW-1
 local y1=7;local y2=math.min(20,e.h-9);if y2<y1+5 then y2=math.min(e.h-5,y1+5)end
 fill(e,x1,y1,x2,y2,C.shell)
 local ix1,ix2=x1+1,x2-1;local iy1,iy2=y1+1,y2-1
 ...
 if filledRows>0 then fill(e,ix1,iy2-filledRows+1,ix2,iy2,C.good)end
 local y=y2+2;if y<=e.h then center(e,y,powerStatus(p),C.text,nil,1,e.w)end
 if y+2<=e.h then center(e,y+2,fmtFE(p.stored,false),C.dim,nil,1,e.w)end
 if y+4<=e.h then put(e,2,y+4,"IN  +"..fmtFE(p.input,true),C.good)end
 if y+5<=e.h then put(e,2,y+5,"OUT -"..fmtFE(p.output,true),C.dim)end
end

function M.init(c)cfg=c or{};return base.init(c)end
function M.render(env,meta)local ok=base.render(env,meta);local mons=detectMonitors();if mons[2]then redrawPower(mons[2],env)end;return ok end
