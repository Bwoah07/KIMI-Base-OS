...
local function power(env)
 local p=state(env).power or{};local best,score=p,-1
 local function consider(x)if type(x)~="table"then return end;local cap=tonumber(x.capacity)or 0;local stored=tonumber(x.stored)or 0;local s=(cap>0 and 1e9 or 0)+(stored>0 and 1e6 or 0)+math.abs(tonumber(x.input)or 0)+math.abs(tonumber(x.output)or 0);if s>score then best,score=x,s end end
 consider(p);for _,x in ipairs(p.matrices or{})do consider(x)end;for _,x in ipairs(p.fluxNetworks or{})do consider(x)end;return best or p
end
...
local function drawHome(env,meta)
 local total,online,current=fleet(env);local ds,ss=doors(env),sensors(env);local p=power(env);local pp=pct(p)
 put(1,4,meta and meta.connected and"BASE ONLINE"or"BASE OFFLINE",meta and meta.connected and C.good or C.warn)
 put(1,6,"JUICE  "..(pp and string.format("%.1f%%",pp)or"NO DATA"),pp and C.good or C.warn)
 put(1,8,"FLEET  "..online.."/"..total.."  CUR "..current,C.text)
 put(1,9,"DOORS  "..#ds,C.text);put(1,10,"SENS   "..#ss,#ss>0 and C.good or C.warn)
 put(1,12,"Use L/R or scroll",C.dim);put(1,13,"Doors page = control",C.accent)
end

local function drawDoors(env)
 doorRows={};local ds=doors(env);local _,h=size();put(1,4,"DOORS  "..#ds,C.text)
 if #ds==0 then put(1,6,"No configured doors",C.warn);return end
 local y=6
 for i,d in ipairs(ds)do
  if y>=h-3 then break end
  local label=clip((i<=9 and tostring(i).." "or"")..upper(d.name or("DOOR "..i)),16)
  local stateText=d.open and"OPEN"or"CLOSED";local fg=d.open and C.good or C.text
  put(1,y,label,fg);put(18,y,stateText,d.open and C.good or C.dim)
  doorRows[#doorRows+1]={y=y,door=d,index=i};y=y+2
 end
 put(1,h-4,"Tap row / keys 1-9",C.accent)
end

local function drawJuice(env)
 local p=power(env);local pp=pct(p);put(1,4,"JUICE / POWER",C.text);put(1,6,pp and string.format("%.1f%%",pp)or"NO DATA",pp and C.good or C.warn);put(1,8,"Stored   "..fmt(p.stored).." FE",C.text);put(1,9,"Capacity "..fmt(p.capacity).." FE",C.dim);put(1,11,"Input   +"..fmt(p.input).." FE/t",C.good);put(1,12,"Output  -"..fmt(p.output).." FE/t",C.warn)
end
...
local function toggleDoor(d,action)
 ...
 local ok,res=action("doors","toggle",args)
 if ok==false then lastStatus="ERR "..clip(tostring(res or"COMMAND FAILED"),20) else lastStatus="DOOR COMMAND SENT" end
 return ok~=false
end
...
function M.handleEvent(ev,env,action)
 if ev[1]=="key" and type(keys)=="table" then
  ...
  elseif page==2 then
   ...
   for i,v in ipairs(nums)do if k==v then idx=i break end end
   if idx then local ds=doors(lastEnv);toggleDoor(ds[idx],action)else return false end
 ...
 elseif ev[1]=="mouse_click" and page==2 then
  local y=tonumber(ev[4]);for _,r in ipairs(doorRows)do if y==r.y or y==r.y+1 then toggleDoor(r.door,action);render(lastEnv,lastMeta);return true end end
 end
 return false
end
return M
