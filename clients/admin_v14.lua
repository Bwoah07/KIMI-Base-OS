...
local function redrawPower(e,env)
 ...
 -- Deliberately simple: one rectangular tank, green filled bottom-up, black empty.
 local x1=4;local x2=math.max(x1+8,e.w-3);local y1=7;local y2=math.min(e.h-8,20)
 if y2<y1+5 then y2=math.min(e.h-5,y1+5)end
 ...
 local y=y2+2;if y<=e.h then center(e,y,fmtFE(p.stored,false),C.text,nil,2,e.w-1)end
 if y+2<=e.h then put(e,2,y+2,"IN  +"..fmtFE(p.input,true),C.good)end
 if y+3<=e.h then put(e,2,y+3,"OUT -"..fmtFE(p.output,true),C.dim)end
end
