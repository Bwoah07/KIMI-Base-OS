...
local function logicalDoorState(d)
 local key=doorKey(d);local p=pendingDoors[key]
 if p then
  if d and d.open==p.desired then pendingDoors[key]=nil;return d.open==true,nil end
  if now()-p.at<=5000 then return p.desired,p end
  pendingDoors[key]=nil
 end
 return d and d.open==true,nil
end
local function drawHome(env,meta)local total,online,current=fleet(env);local ds,ss=doors(env),sensors(env);local p=power(env);local pp=pct(p);put(1,4,meta and meta.connected and"BASE ONLINE"or"BASE OFFLINE",meta and meta.connected and C.good or C.warn);put(1,6,"JUICE  "..(pp and string.format("%.1f%%",pp)or"NO DATA"),pp and C.good or C.warn);put(1,8,"FLEET  "..online.."/"..total.."  CUR "..current,C.text);put(1,9,"DOORS  "..#ds,C.text);put(1,10,"SENS   "..#ss,#ss>0 and C.good or C.warn);put(1,12,"L/R or scroll pages",C.dim);put(1,13,"DOORS = remote control",C.accent)end
local function drawDoors(env)doorRows={};local ds=doors(env);local _,h=size();put(1,4,"DOORS  "..#ds,C.text);if #ds==0 then put(1,6,"No configured doors",C.warn);return end;local y=6;for i,d in ipairs(ds)do if y>=h-3 then break end;local opened,p=logicalDoorState(d);local label=clip((i<=9 and tostring(i).." "or"")..upper(d.name or("DOOR "..i)),15);local stateText=p and(p.desired and"OPENING"or"CLOSING")or(opened and"OPEN"or"CLOSED");local color=p and C.warn or(opened and C.good or C.dim);put(1,y,label,opened and C.good or C.text);put(17,y,stateText,color);doorRows[#doorRows+1]={y=y,door=d,index=i};y=y+2 end;put(1,h-4,"Tap row / keys 1-9",C.accent)end
...
local function commandDoor(d,action)
 ...
 local key=doorKey(d);local current=select(1,logicalDoorState(d));local desired=not current;local t=now()
 if t-(lastDoorTap[key]or-10000)<250 then lastStatus="WAIT";return false end
 lastDoorTap[key]=t
 local command=desired and"open"or"close"
 local args={_source=d._source or d.source,target=d.target,side=d.side,id=d.id,key=d.key}
 local ok,res=action("remote_doors",command,args)
 if ok==false then pendingDoors[key]=nil;lastStatus="ERR "..clip(tostring(res or"COMMAND FAILED"),20);return false end
 pendingDoors[key]={desired=desired,at=t}
 lastStatus=desired and"OPEN COMMAND SENT"or"CLOSE COMMAND SENT"
 return true
end
function M.init(c)cfg=c or{};pendingDoors={};lastDoorTap={};clear()end
...
function M.handleEvent(ev,env,action)
 if ev[1]=="key" and type(keys)=="table" then
  ...
  elseif page==2 then
   ...
   if idx then local ds=doors(lastEnv);commandDoor(ds[idx],action)else return false end
 ...
 elseif ev[1]=="mouse_click" and page==2 then
  local y=tonumber(ev[4]);for _,r in ipairs(doorRows)do if y==r.y or y==r.y+1 then commandDoor(r.door,action);render(lastEnv,lastMeta);return true end end
 end
 return false
end
return M
