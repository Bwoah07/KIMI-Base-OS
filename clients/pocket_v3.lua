...
local function visibleState(d)local key=doorKey(d);local p=pending[key];if p then if d and d.open==p.desired then pending[key]=nil;return d.open==true,nil end;if now()-p.at<=4000 then return p.desired,p end;pending[key]=nil end;return d and d.open==true,nil end
local function sendDoor(d,desired,action,isRetry)
