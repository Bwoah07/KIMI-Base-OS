...
local function generatedName() return "KIMI-" .. tostring(os.getComputerID()) end
...
local defaults = {
    ...
    update = { channel="alpha", auto=true, fleetManaged=true, checkOnBoot=true, interval=600 }
}

local function merge(dst,src)
    for k,v in pairs(src or {}) do if type(v)=="table" and type(dst[k])=="table" then merge(dst[k],v) else dst[k]=v end end
    return dst
end
...
function M.load()
    ...
    if label and (not cfg.name or cfg.name=="" or cfg.name==generatedName()) then cfg.name=label end
    return cfg
end

function M.save(cfg)
    if not fs.exists(ROOT) then fs.makeDir(ROOT) end
    local f=assert(fs.open(PATH,"w")); f.write(textutils.serialize(cfg)); f.close()
end
