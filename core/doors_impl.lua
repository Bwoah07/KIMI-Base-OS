...
local function computerOutput(side)
  if not has(computerSides,side) then return false,false end
...
local function setActuator(c,side,value)
  if c.target=="computer" then
    if not has(computerSides,side) then return false,"invalid computer redstone side" end
  ...
  if c.kind=="digital_side" then
    if not has(worldSides,side) then return false,"invalid redstone actuator side" end
    return pcallPeripheral(c.target,"setOutput",side,value==true)
  elseif c.kind=="analog_side" then
    if not has(worldSides,side) then return false,"invalid analog actuator side" end
  ...
  elseif c.kind=="enabled_actuator" then return pcallPeripheral(c.target,"setEnabled",value==true)
  elseif c.kind=="active_actuator" then return pcallPeripheral(c.target,"setActive",value==true) end
...
local function feedbackState(d,c)
  ...
  local side=tostring(d.feedbackSide)
  local value,ok
...
local function logicalState(d,c,physical)
  local fb,ok=feedbackState(d,c)
  if ok then return fb,"feedback" end
