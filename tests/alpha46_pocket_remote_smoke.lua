...
local env={version="5.0.0-alpha.63",state={
 ...
 fleet={[1]={online=true,version="5.0.0-alpha.63",name="Main Base"},[42]={online=true,version="5.0.0-alpha.63",name="Vault Room"},[77]={online=true,version="5.0.0-alpha.63",name="Pocket Boss"}}
}}
local meta={connected=true,localVersion="5.0.0-alpha.63"}
...
assert(called.args._source==nil and tostring(called.args.source)=="42" and called.args.target=="computer" and called.args.side=="left","pocket lost safe door owner/actuator routing")
...
-- Remote router must add destination _source only after Main Base has resolved it.
package.loaded["core.network"]={send=function(target,cfg,kind,payload)
 ...
 assert(tostring(payload.args._source)=="42","remote router did not add destination ownership")
 return true
end}
...
local result=r.handleCommand("open",{source="42",target="computer",side="left"})
