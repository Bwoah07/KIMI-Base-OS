...
local matrix={stored=1000,capacity=1000,input=100,output=25,filledPercentage=1}
local env={version="5.0.0-alpha.61",state={doors={doors={}},power={matrices={matrix}},attachments={sensors={}},fleet={}}}
...
assert(limeRows>=8 and limeLast-limeFirst+1==limeRows,"100% rectangle fill must be continuous")
assert(limeWidth>=5 and limeWidth<=7,"rectangle fill width regressed")
assert(shellRows>=10 and shellWidth>=7 and shellWidth<=9,"rectangle border width regressed")
...
assert(lowRows>=1 and lowRows<limeRows and lowWidth>=5,"20% charge did not reduce green fill");assert(redRows==0,"simple Matrix rectangle must stay green")
