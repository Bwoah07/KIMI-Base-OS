...
-- Pocket: home exposes a one-tap door, retries are idempotent, telemetry acknowledgement reverses next action.
local W,H=26,20
local rows,x,y={},1,1
...
p.render(env,meta)
local out=output()
assert(out:find("BASE ONLINE",1,true)and out:find("VAULT",1,true)and out:find("TAP TO CLOSE",1,true),"Pocket home lost instant door control")
local calls={}
