...
-- Pocket invariant: an immediately visible door control sends explicit commands and stale telemetry reverses optimistically.
local W,H=26,20
local rows,x,y={},1,1
...
local env={version="5.0.0-alpha.61",state={doors={doors={{id="D1",name="VAULT",_source="42",target="computer",side="left",open=true,online=true}}},power={stored=800,capacity=1000,input=50,output=20,filledPercentage=.8},attachments={sensors={}},fleet={}}}
...
epoch=1200;p.handleEvent({"mouse_click",1,3,12},env,action);assert(#calls==2 and calls[2].cmd=="open","Pocket did not use optimistic state for immediate reverse")
...
local aenv={version="5.0.0-alpha.61",state={doors={doors={{name="FRONT GATE",open=true,online=true},{name="ROOM PANEL",open=false,online=true}}},power={matrices={{stored=1000,capacity=1000,input=32000,output=22000,filledPercentage=1}}},attachments={sensors={}},fleet={}}}
