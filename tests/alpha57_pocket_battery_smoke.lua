...
-- Pocket invariant: home exposes an immediate explicit door control and stale telemetry does not repeat the same command.
local W,H=26,20
local rows,x,y={},1,1
...
package.loaded["clients.pocket_v4"]=nil;package.loaded["clients.pocket_v3"]=nil;package.loaded["clients.pocket"]=nil
local p=assert(loadfile("clients/pocket.lua"))();p.init({})
local env={version="5.0.0-alpha.60",state={doors={doors={{id="D1",name="VAULT",_source="42",target="computer",side="left",open=true,online=true}}},power={stored=800,capacity=1000,input=50,output=20,filledPercentage=.8},attachments={sensors={}},fleet={}}}
local meta={connected=true}
p.render(env,meta)
local out=output()
assert(out:find("BASE ONLINE",1,true)and out:find("VAULT",1,true)and out:find("CLOSE DOOR",1,true),"Pocket home lost instant door control")
...
p.handleEvent({"mouse_click",1,3,13},env,action)
assert(#calls==1 and calls[1].cmd=="close","Pocket did not send explicit CLOSE")
epoch=1200
p.handleEvent({"mouse_click",1,3,13},env,action)
assert(#calls==2 and calls[2].cmd=="open","Pocket did not use optimistic state for immediate reverse command")

-- Admin invariant: dedicated power display remains a large readable green vertical fill; center keeps door states without duplicated telemetry.
...
local aenv={version="5.0.0-alpha.60",state={doors={doors={{name="FRONT GATE",open=true,online=true},{name="ROOM PANEL",open=false,online=true}}},power={stored=1000,capacity=1000,input=32000,output=22000,filledPercentage=1},attachments={sensors={}},fleet={[1]={name="MAIN BASE",online=true,version="5.0.0-alpha.60"},[2]={name="ROOM PANEL",online=true,version="5.0.0-alpha.60"}}}}
assert(admin.render(aenv,{localServer=true})~=false,"admin render failed")
local greenRows,greenWidth=powerMon.greenShape()
assert(greenRows>=10,"battery fill is too short to read clearly")
assert(greenWidth>=15,"battery rectangle is too narrow")
