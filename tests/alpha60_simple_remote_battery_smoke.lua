...
local aenv={version="5.0.0-alpha.71",state={doors={doors={{name="ROOM PANEL",open=false,online=true}}},environment={online=true,weather="CLEAR"},power={matrices={{peripheral="matrix_main",stored=500,capacity=1000,input=123,output=45,filledPercentage=.5}},fluxNetworks={{networkName="GRID",stored=1000,capacity=1000,filledPercentage=1}}},power_reserve={status="NO RESERVE MATRIX"},attachments={devices={},sensors={}},fleet={}}}
assert(admin.render(aenv,{localServer=true})~=false,"admin render failed")
local greenRows,greenWidth=powerMon.greenShape();assert(greenRows>=2 and greenRows<=4,"50% Matrix should fill roughly half the compact gauge");assert(greenWidth>=5 and greenWidth<=7,"Matrix gauge is not narrow");realPrint("alpha60 simple remote/battery compatibility test OK")
